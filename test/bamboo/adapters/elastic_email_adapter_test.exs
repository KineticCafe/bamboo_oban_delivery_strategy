defmodule Bamboo.ObanDeliveryStrategyTest do
  use ExUnit.Case
  alias Bamboo.ObanDeliveryStrategy
  alias Bamboo.Email
  alias Bamboo.Mailer
  alias Plug.Adapters.Cowboy
  alias Plug.Conn

  @config %{adapter: ObanDeliveryStrategy, api_key: "123_abc"}
  @config_with_bad_key %{adapter: ObanDeliveryStrategy, api_key: nil}
  ...
  test "raises if the api key is nil" do
    assert_raise ArgumentError, ~r/no API key set/, fn ->
      [from: "foo@bar.com"]
      |> new_email()
      |> ObanDeliveryStrategy.deliver(@config_with_bad_key)
    end

    assert_raise ArgumentError, ~r/no API key set/, fn ->
      ObanDeliveryStrategy.handle_config(%{})
    end
  end
  ...
  describe "when sending emails" do
    ...
    test "deliver/2 sends the to the right url" do
      new_email() |> ObanDeliveryStrategy.deliver(@config)
    ...
    test "deliver/2 sends from, html and text body, subject, reply_to, and headers" do
      ...
      ObanDeliveryStrategy.deliver(email, @config)
    ...
    test "deliver/2 adds extra params to the message " do
      ObanDeliveryStrategy.deliver(new_email(), @config)
    ...
    test "raises if the response is not a success" do
      ...
      assert_raise Bamboo.ObanDeliveryStrategy.ApiError, fn ->
        email |> ObanDeliveryStrategy.deliver(@config)
      end
    end
    ...
    test "removes api key from error output" do
      ...
      assert_raise Bamboo.ObanDeliveryStrategy.ApiError, ~r/"apikey" => "\[FILTERED\]"/, fn ->
        ObanDeliveryStrategy.deliver(email, @config)
      end
    end
